  ...
  return (
    <div className="flex flex-col h-full bg-app-bg text-app-text animate-fade-in relative">
      <PageHeader
        ...
        title="Notas Financieras NIIF & NIC 12"
        badge={
        ...
        subtitle={`Cumplimiento contable bajo la norma internacional NIC 12 y notas dinámicas estructuradas • RUC: ${currentCompany?.ruc || ''}`}
        actions={
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-bold text-app-muted uppercase">Período Contable:</span>
              <select
            ...
            {isClosed && (
              <span className="flex items-center gap-1 text-[9px] font-black uppercase tracking-wider bg-amber-500/10 text-amber-400 border border-amber-500/30 px-2.5 py-1.5 rounded-xl">
                <AlertTriangle size={12} /> Cerrado
              </span>
            )}
          </div>
        }
      ...
      {/* TAB SYSTEM */}
      <div className="flex bg-app-bg p-1.5 rounded-xl border border-app-border w-full sm:w-fit overflow-x-auto custom-scrollbar whitespace-nowrap">
        <button
      ...
      {activeSubTab === 'NIC12' ? (
        <div className="space-y-6">
          ...
          {/* PARAMETERS BLOCK */}
          <div className="bg-app-surface/50 backdrop-blur-sm p-4 rounded-xl border border-app-border flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="bg-app-surface/70 backdrop-blur-md rounded-xl border border-app-border shadow-xl overflow-hidden">
            <div className="p-4 border-b border-app-border">
              <h3 className="text-md font-bold text-app-text">Conciliación de Diferencias Temporarias</h3>
            ...
            <div className="overflow-x-auto custom-scrollbar">
              <table className="w-full text-left border-collapse min-w-[1000px]">
                <thead>
                  <tr className="bg-app-bg/60 text-app-muted text-[10px] font-bold uppercase border-b border-app-border">
                    <th className="px-3 py-2">Concepto</th>
                    <th className="px-3 py-2 text-center">Cuenta</th>
                    <th className="px-3 py-2 text-right">Base Contable (NIIF)</th>
                    <th className="px-3 py-2 text-right">Base Tributaria (LIR)</th>
                    <th className="px-3 py-2 text-right">Diferencia</th>
                    <th className="px-3 py-2 text-center">Tipo</th>
                    <th className="px-3 py-2 text-right">Activo Diferido</th>
                    <th className="px-3 py-2 text-right">Pasivo Diferido</th>
                ...
                <tbody className="divide-y divide-app-border/80 text-sm">
                  {schedule.rows.map((row) => (
                    <tr key={row.key} className="hover:bg-app-hover transition-colors">
                      <td className="px-3 py-1.5 font-semibold text-app-text max-w-xs truncate">{row.concepto}</td>
                      <td className="px-3 py-1.5 text-center text-app-muted font-mono">{row.cuenta}</td>
                      <td className="px-3 py-1.5 text-right text-app-text font-semibold">
                      ...
                      <td className="px-3 py-1.5 text-right">
                        <input
                          ...
                          className="w-28 bg-app-bg border border-app-border rounded-lg px-2 py-1 text-right font-medium focus:outline-none focus:ring-1 focus:ring-blue-500 text-app-text text-xs"
                        />
                      </td>
                      <td className="px-3 py-1.5 text-right text-amber-400 font-semibold">
                      ...
                      <td className="px-3 py-1.5 text-right text-emerald-400 font-bold">
                  ...
                  {/* TOTALS ROW */}
                  <tr className="bg-app-bg/80 font-bold text-app-text border-t-2 border-app-border text-xs">
                    <td className="px-3 py-2" colSpan={6}>TOTALES</td>
                    <td className="px-3 py-2 text-right text-emerald-400 font-extrabold">
                      {new Intl.NumberFormat('es-PE', { style: 'currency', currency: 'PEN' }).format(schedule.totalActivoDiferido)}
                    </td>
                    <td className="px-3 py-2 text-right text-red-400 font-extrabold">
          ...
          {!isClosed && (
            <div className="bg-app-surface/50 backdrop-blur-sm p-4 rounded-xl border border-app-border space-y-3">
          ...
          <div className="bg-app-surface/70 backdrop-blur-md p-4 rounded-xl border border-app-border space-y-2">
            <h4 className="text-xs font-bold uppercase tracking-wider text-app-muted">Asiento Sugerido NIC 12</h4>
            <div className="bg-app-bg p-3 rounded-lg font-mono text-[11px] border border-app-border divide-y divide-app-border/80">
              {schedule.suggestedJournalEntries.map((e, index) => (
                <div key={index} className="py-1.5 flex justify-between items-center text-app-muted">
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 border-b border-app-border pb-4">
            <div className="space-y-0.5">
              <h3 className="text-md font-bold text-app-text">Notas a los Estados Financieros</h3>
            ...
            <div className="flex gap-2 flex-wrap">
              <button
                onClick={handlePrint}
                className="bg-app-surface hover:bg-app-hover text-app-text font-bold px-4 py-2 rounded-xl border border-app-border transition flex items-center gap-2 text-xs"
              >
              ...
              <button
                ...
                disabled={isClosed}
                className="bg-blue-600 hover:bg-blue-500 text-white font-bold px-4 py-2 rounded-xl transition flex items-center gap-2 shadow-lg text-xs"
              >
          ...
          <div className="grid grid-cols-1 gap-4">
            {showNotesWithTables.map((note) => (
              <div
                key={note.number}
                className="bg-app-surface/70 backdrop-blur-md p-4 rounded-xl border border-app-border shadow-md space-y-3 print:bg-white print:text-black print:border-none print:shadow-none print:p-0"
              >
                {/* Note title */}
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full bg-app-bg border border-app-border rounded-lg p-3 text-xs text-app-text focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 leading-relaxed font-normal print:bg-transparent print:border-none print:text-black print:p-0 print:resize-none"
                />

                {/* Table Data Preview (If exist) */}
                {note.tableData && (
                  <div className="overflow-x-auto rounded-lg border border-app-border bg-app-bg/30 print:border-black print:bg-transparent">
                      ...
                      <tbody className="divide-y divide-app-border/60 print:divide-black">
                        {note.tableData.rows.map((row, ri) => (
                          <tr key={ri} className="text-app-muted print:text-black">
                            {row.map((cell, ci) => (
                              <td key={ci} className={`px-3 py-1.5 ${ci === row.length - 1 ? 'font-bold text-right' : ''}`}>{cell}</td>
                            ))}
                          </tr>
                        ))}
                      </tbody>
        ...
        <div className="space-y-6">
          <div className="bg-app-surface/70 backdrop-blur-md rounded-2xl border border-app-border shadow-xl overflow-hidden">
            <div className="p-4 border-b border-app-border">
              <h3 className="text-md font-bold text-app-text">Conciliación Tributaria: Utilidad Contable → Renta Neta Imponible</h3>
            ...
            <div className="p-4 border-b border-app-border">
              <h4 className="text-xs font-black uppercase tracking-wider text-red-400 mb-3 flex items-center gap-2">
                <span className="w-2.5 h-2.5 bg-red-500 rounded-full"></span> (+) ADICIONES A LA RENTA NETA
              </h4>
              <div className="space-y-2">
                {adicDedItems.filter(i => i.tipo === 'ADICION').map(item => (
                  <div key={item.id} className="flex flex-col sm:flex-row sm:items-center gap-2 p-2 rounded-lg bg-red-500/5 border border-red-500/10 hover:bg-red-500/10 transition-colors justify-between">
                    ...
                    <input
                      ...
                      className="w-full sm:w-28 bg-app-bg border border-app-border rounded-lg px-2 py-1 text-right text-xs font-bold text-red-400 focus:outline-none focus:ring-1 focus:ring-red-500"
                    />
                  </div>
                ))}
              </div>
            ...
            <div className="p-4 border-b border-app-border">
                <h4 className="text-xs font-black uppercase tracking-wider text-emerald-400 mb-3 flex items-center gap-2">
                  <span className="w-2.5 h-2.5 bg-emerald-500 rounded-full"></span> (−) DEDUCCIONES A LA RENTA NETA
                </h4>
              <div className="space-y-2">
                {adicDedItems.filter(i => i.tipo === 'DEDUCCION').map(item => (
                  <div key={item.id} className="flex flex-col sm:flex-row sm:items-center gap-2 p-2 rounded-lg bg-emerald-500/5 border border-emerald-500/10 hover:bg-emerald-500/10 transition-colors justify-between">
                    <div className="flex-1">
                      <p className="text-xs font-bold text-app-text">{item.concepto}</p>
                      <p className="text-[9px] text-app-muted">{item.base} • {item.permanente ? 'Permanente' : 'Temporal'}</p>
                    </div>
                    <input
                      ...
                      disabled={isClosed}
                      onChange={(e) => setAdicDedItems(prev => prev.map(p => p.id === item.id ? { ...p, monto: Number(e.target.value) } : p))}
                      className="w-full sm:w-28 bg-app-bg border border-app-border rounded-lg px-2 py-1 text-right text-xs font-bold text-emerald-400 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                    />
                  </div>
                ))}
              </div>
            ...
            {(() => {
              ...
              return (
                <div className="p-4 space-y-2">
                  <div className="flex justify-between items-center px-3 py-2 bg-app-bg/30 rounded-lg border border-app-border/50">
                    <span className="text-[11px] font-bold text-app-muted uppercase">Utilidad (Pérdida) Contable del Ejercicio</span>
                    <span className={`text-xs font-black ${utilidadContable >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{fmt(utilidadContable)}</span>
                  </div>
                  <div className="flex justify-between items-center px-3 py-2 bg-red-500/5 rounded-lg border border-red-500/10">
                    <span className="text-[11px] font-bold text-red-400 uppercase">(+) Total Adiciones</span>
                    <span className="text-xs font-black text-red-400">{fmt(totalAdiciones)}</span>
                  </div>
                  <div className="flex justify-between items-center px-3 py-2 bg-emerald-500/5 rounded-lg border border-emerald-500/10">
                    <span className="text-[11px] font-bold text-emerald-400 uppercase">(−) Total Deducciones</span>
                    <span className="text-xs font-black text-emerald-400">{fmt(totalDeducciones)}</span>
                  </div>
                  <div className="flex justify-between items-center px-3 py-2.5 bg-gradient-to-r from-amber-500/10 to-orange-500/10 rounded-lg border border-amber-500/20">
                    <span className="text-xs font-black text-amber-500 uppercase tracking-wider">RENTA NETA IMPONIBLE</span>
                    <span className="text-lg font-black text-amber-500">{fmt(rentaNetaFinal)}</span>
                  </div>
                  <div className="flex justify-between items-center px-3 py-2.5 bg-gradient-to-r from-indigo-500/10 to-blue-500/10 rounded-lg border border-indigo-500/20">
                    <div>
                    ...
                    <span className="text-lg font-black text-indigo-400">{fmt(ir)}</span>
                  </div>
                </div>
              );
            })()}
        ...
        /* ITAN TAB */
        <div className="bg-app-surface/70 backdrop-blur-md rounded-xl border border-app-border overflow-hidden shadow-2xl p-4 space-y-4">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 border-b border-app-border pb-3">
            <div>
              <h3 className="text-md font-black text-app-text flex items-center gap-2">
            ...
            <button
              onClick={handleSaveNIC12}
              className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-xs font-bold shadow-lg shadow-purple-600/20 hover:scale-105 active:scale-95 transition-all"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="card-elevated !p-4 bg-app-bg border border-app-border space-y-3">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full bg-app-surface border border-app-border rounded-lg px-3 py-1.5 text-sm font-black text-app-text text-right focus:outline-none focus:ring-2 focus:ring-purple-500"
                />
            ...
            <div className="md:col-span-2 card-elevated !p-4 bg-app-bg border border-app-border space-y-3">
                  ...
                  <tbody className="divide-y divide-app-border/30">
                    {itanCuotas.map((c, idx) => (
                      <tr key={idx} className="hover:bg-app-surface/20">
                        <td className="py-1.5 font-bold">Cuota N° 0{c.cuota}</td>
                        ...
                        <td className="py-1.5 text-center">
                          <button
                            ...
                            className={`px-3 py-0.5 rounded-lg font-bold text-[8px] uppercase tracking-wider transition ${
                        ...
                        <td className="py-1.5">
                          <input
                            ...
                            className="bg-app-surface border border-app-border rounded-lg px-1.5 py-0.5 text-[9px] w-28 focus:outline-none text-app-text"
                          />
                        </td>
                        <td className="py-1.5">
                          <input
                            ...
                            className="bg-app-surface border border-app-border rounded-lg px-1.5 py-0.5 text-[9px] w-24 focus:outline-none text-app-text uppercase font-mono"
                          />
                        </td>
                      </tr>
                    ))}
