...
export const Login: React.FC = () => {
    ...
    return (
        <div className="min-h-screen h-screen w-screen bg-gradient-to-br from-[#0d2238] via-[#122b47] to-[#091829] flex items-center justify-center p-2 sm:p-4 lg:p-5 font-sans selection:bg-blue-600/20 selection:text-blue-900 relative overflow-hidden">
            ...
            {/* ═══════════════════════════════════════════════════════════════ */}
            {/* TARJETA UNIFICADA CON GLASSMORPHISM REAL Y RESPONSIVE MÓVIL    */}
            {/* ═══════════════════════════════════════════════════════════════ */}
            <div className="relative z-10 w-full max-w-md lg:max-w-[860px] xl:max-w-[940px] 2xl:max-w-[1020px] h-auto lg:h-[88vh] lg:max-h-[580px] xl:max-h-[630px] bg-white/40 lg:bg-white/35 backdrop-blur-2xl rounded-3xl shadow-[0_25px_80px_rgba(0,0,0,0.45),inset_0_1px_2px_rgba(255,255,255,0.7)] border border-white/60 overflow-hidden flex flex-col lg:flex-row items-stretch my-auto animate-fade-in">
                
                {/* LADO IZQUIERDO: Formulario de Autenticación con Glassmorphism */}
                <div className="w-full lg:w-[380px] xl:w-[410px] bg-white/30 lg:bg-transparent backdrop-blur-sm lg:backdrop-blur-none p-5 sm:p-6 xl:p-7 flex flex-col justify-between overflow-y-auto login-scroll shrink-0">
                    
                    {/* Header Marca */}
                    <div className="text-center mb-2">
                        <div className="inline-flex items-center justify-center p-2 bg-white/80 backdrop-blur-md border border-white/90 rounded-2xl shadow-sm mb-1.5">
                        ...
                        <p className="text-slate-600 text-[9.5px] font-extrabold tracking-[0.2em] uppercase mt-0.5 notranslate" translate="no">
                    ...
                    <div className="flex-1 flex flex-col justify-center py-1">
                        ...
                        <div className="mb-2.5 bg-white/60 backdrop-blur-md p-1 rounded-2xl border border-white/80 flex items-center gap-1 select-none shadow-sm">
                            <button
                                type="button"
                                onClick={() => {
                                ...
                                className={`flex-1 py-1.5 px-2.5 rounded-xl text-[9.5px] font-black uppercase tracking-wider transition-all duration-200 flex items-center justify-center gap-1.5 cursor-pointer outline-none focus:outline-none ${
                                    !isStudentModeActive 
                                        ? 'bg-blue-600 text-white shadow-md shadow-blue-600/30 scale-[1.02]' 
                                        : 'text-slate-600 hover:text-slate-900 hover:bg-white/60'
                                }`}
                            >
                                <Building2 size={13} className={!isStudentModeActive ? 'text-white' : 'text-slate-500'} />
                            ...
                            >
                                <GraduationCap size={14} className={isStudentModeActive ? 'text-white' : 'text-slate-500'} />
                                <span>Estudiante</span>
                            </button>
                        </div>

                        {/* Banner informativo del modo */}
                        <div className={`mb-2.5 p-2 rounded-xl border text-[9.5px] font-semibold flex items-center gap-1.5 shadow-sm ${
                            isStudentModeActive
                                ? 'bg-indigo-500/15 border-indigo-300/40 text-indigo-950'
                                : 'bg-blue-500/15 border-blue-300/40 text-blue-950'
                        }`}>
                            {isStudentModeActive ? (
                                <>
                        ...
                        <div className="flex mb-2.5 border-b border-white/60 pb-1">
                            <button 
                                type="button"
                                onClick={() => {
                                    setIsLogin(true);
                                ...
                                className={`flex-1 py-1 text-xs font-extrabold uppercase tracking-wider border-b-2 transition-all cursor-pointer ${
                                    isLogin 
                                        ? isStudentModeActive ? 'border-indigo-600 text-indigo-900 font-black' : 'border-blue-600 text-blue-900 font-black' 
                        ...
                        <form onSubmit={handleSubmit} className="space-y-2.5" autoComplete="on">
                            {errorAlert && (
                                <div className="bg-rose-50/90 border border-rose-200 text-rose-800 text-xs py-2 px-2.5 rounded-xl flex items-start gap-2 animate-in fade-in duration-200">
                                    <span className="text-rose-600 mt-0.5 text-xs shrink-0">⚠️</span>
                                    <div className="flex-1 font-semibold leading-tight text-[11px]">{errorAlert}</div>
                                </div>
                            )}

                            {!isLogin && (
                                <div className="space-y-0.5">
                                    <label className="text-[9.5px] font-black text-slate-700 ml-1 uppercase tracking-wider">Nombre Completo</label>
                                    <div className={`relative flex items-center rounded-xl bg-white/80 backdrop-blur-md border border-white/90 shadow-sm focus-within:bg-white focus-within:border-blue-500 transition-all ${isStudentModeActive ? 'light-input-field-student' : ''}`}>
                                        ...
                                        <input 
                                            ...
                                            className="w-full py-2 pr-3 bg-transparent placeholder:text-slate-400 text-xs text-slate-900 focus:outline-none font-medium"
                            ...
                            <div className="space-y-0.5">
                                <label className="text-[9.5px] font-black text-slate-700 ml-1 uppercase tracking-wider">Correo Electrónico</label>
                                <div className={`relative flex items-center rounded-xl bg-white/80 backdrop-blur-md border border-white/90 shadow-sm focus-within:bg-white focus-within:border-blue-500 transition-all ${isStudentModeActive ? 'light-input-field-student' : ''}`}>
                            ...
                            <div className="space-y-0.5">
                                <div className="flex items-center justify-between">
                                    <label className="text-[9.5px] font-black text-slate-700 ml-1 uppercase tracking-wider">Contraseña</label>
                                ...
                                <div className={`relative flex items-center rounded-xl bg-white/80 backdrop-blur-md border border-white/90 shadow-sm focus-within:bg-white focus-within:border-blue-500 transition-all ${
                                    !isLogin && checkPasswordStrength(formData.password).isValid
                                        ? '!border-2 !border-emerald-500 !bg-emerald-50/40 ring-2 ring-emerald-500/20'
                                    ...
                                    <input 
                                        ...
                                        className="w-full py-2 bg-transparent placeholder:text-slate-400 text-xs text-slate-900 focus:outline-none font-medium"
                            ...
                            {isLogin && (
                                <div className="flex items-center justify-end px-1 pt-0.5 text-xs">
                                    <button 
                                        ...
                                        className="text-blue-700 hover:text-blue-900 transition-colors font-extrabold cursor-pointer hover:underline text-[10.5px]"
                                    >
                                        ¿Olvidaste tu clave?
                                    </button>
                                </div>
                            ...
                            <button 
                                type="submit"
                                disabled={isLoading}
                                className={`w-full font-black py-2.5 rounded-xl shadow-lg transition-all duration-200 flex items-center justify-center gap-2 mt-2 cursor-pointer text-xs uppercase tracking-wider ${
                                    isStudentModeActive
                                        ? 'bg-gradient-to-r from-indigo-600 via-indigo-700 to-purple-700 hover:from-indigo-700 hover:to-purple-800 text-white shadow-indigo-600/30'
                                        : 'bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-700 hover:from-blue-700 hover:to-indigo-800 text-white shadow-blue-600/30'
                    ...
                    <div className="mt-2 text-center space-y-0.5">
                        <p className="text-slate-600 text-[9.5px] font-medium">
                            Al ingresar aceptas nuestros{' '}
                            <button type="button" onClick={() => setShowLoginLegal('terms')} className="text-slate-800 hover:text-blue-700 underline font-bold cursor-pointer transition-colors">Términos</button>{' '}y{' '}
                            <button type="button" onClick={() => setShowLoginLegal('privacy')} className="text-slate-800 hover:text-blue-700 underline font-bold cursor-pointer transition-colors">Privacidad</button>.
                        </p>
                        <p className="text-slate-500 text-[9px] font-bold tracking-wider notranslate" translate="no">
                            &copy; 2026 Angelo Thomas Serna Simeon. SOFTCONTABLE SaaS.
                        </p>
                    </div>
                </div>

                {/* LADO DERECHO: Hero Showcase — Unificado con Glassmorphism y 100% visible en laptops */}
                <div className="hidden lg:flex flex-1 bg-white/20 backdrop-blur-md items-center justify-center p-2 xl:p-3 overflow-hidden relative">
                    <img 
